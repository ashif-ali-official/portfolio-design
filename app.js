...
for(let i=0; i<365; i++){
    ...
    div.classList = list.includes(i) ? "box active" : "box";
...
window.addEventListener("scroll", () => {
    ...
    phones[0].style.transform = `translateX(calc(475vh - ${offsetY}px))`;
    laptop.style.transform = `translateX(calc(-600vh + ${offsetY}px))`;
    phones[1].style.transform = `translateX(calc(725vh - ${offsetY}px))`;
})
